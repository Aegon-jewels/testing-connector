...
Now automatically analyzes the captured pcap and extracts STUN/RTP info.
"""
...
class NetworkCapture:
    ...
    def _analyze_pcap(self) -> dict:
        """Run tshark on the captured pcap and extract useful information."""
        ...
        try:
            ...
            result["udp_conversations"] = output
        except Exception:
            result["udp_conversations"] = "Could not extract UDP conversations."

        # ---- 2. Extract STUN packets and XOR-MAPPED-ADDRESS ----
        cmd = [
            "tshark", "-r", pcap,
            "-Y", "stun",
            "-T", "fields",
                ...
                if line.strip():
                    ...
                    if len(parts) >= 5:
                        stun_entries.append({
                            ...
                            "src_port": parts[2],
                            "dst_port": parts[3],
                            "xored_ip": parts[4] if parts[4] else "N/A"
                        })
            result["stun_packets"] = stun_entries
        except Exception:
            result["stun_packets"] = []

        # ---- 3. RTP stream summary (if any) ----
        cmd = [
            "tshark", "-r", pcap,
            "-q", "-z", "rtp,streams"
        ]
    ...
    def export_to_file(self, filename: str):
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            section("UDP PACKET ANALYSIS (from pcap)")
            pcap = r.get("pcap_analysis", {})
            W("  STUN Packets (NAT traversal):\n")
            for stun in pcap.get("stun_packets", []):
                W(f"    {stun['src_ip']}:{stun['src_port']} → {stun['dst_ip']}:{stun['dst_port']}  XOR‑IP: {stun['xored_ip']}\n")
            if not pcap.get("stun_packets"):
                W("    No STUN packets found.\n")
            W("\n  UDP Conversations (tshark conv,udp):\n")
    ...
    def export_analysis_json(self, filename: str):
        """Export only the pcap analysis to a JSON file."""
        r = self.get_report()
